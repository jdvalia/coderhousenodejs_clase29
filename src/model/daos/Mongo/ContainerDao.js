...
export default class ContainerDao {
    ...
    async add(data) {
        try {
            ...
            throw new CustomError(500, `Error agregando documento ${this.collectionName}`, err)
        }
    }
    async update(id, query) {
        try {
            ...
            throw new CustomError(500, `Error actualizando documento ${this.collectionName}`, err)
        }
    }
    async deleteById(id) {
        let query = { "id": id };
        await this.collection.deleteOne(query, function (err, obj) {
            if (err) {
                throw new CustomError(500, `Error borrando documento ${this.collectionName}`, err)
            }
        });
    }
}